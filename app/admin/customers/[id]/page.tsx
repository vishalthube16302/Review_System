...
export default function EditCustomerPage({ params }: { params: { id: string } }) {
  ...
  if (loading || !business) return <div className="p-6">Loading...</div>
      ...
      <form onSubmit={handleUpdate} className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 bg-slate-100 rounded-lg px-4 py-3">
            ...
            <span className="ml-auto text-xs text-slate-400">🔒 Linked to printed QR code</span>
          </div>
        </div>
        ...
        <div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) => set('is_active', e.target.checked ? 'true' : 'false')}
              className="w-4 h-4"
            />
            <span className="text-sm">Active (QR code works)</span>
          </label>
        </div>
